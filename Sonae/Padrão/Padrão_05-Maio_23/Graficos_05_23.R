...
#Dados todos
Dados <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\FundidoFinal.xlsx")
Dados2 <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_2_DataFusion_2022.xlsx")
...
df_Classic <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_5_med1.xlsx")
df_ClassicRep <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_5_med_reposto1.xlsx")
df_ClassicSemRep <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_5_med_naoreposto1.xlsx")

df_NumProdutos <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_5_NumProdutos1.xlsx")

df_Rotura <- read_excel("D:\\B&N Dados\\Delta\\Padrão\\Padrão_05_2023\\OUT_5_Rotura1.xlsx")
...
#Ordem dos elementos
#1º Elemento: CAFÉ DELTA Q QALIDUS 10CAP
#Último elemento: BEBIDA CEREAIS DELTA C/20%CAFE FR 200G


#Nº de Produtos: 11
#produto1 <-  "#9e2f24 CAFÉ DELTA Q QALIDUS 10CAP"
#produto2 <-  "#662867 CAFÉ DELTA Q MYTHIQ 10CAP"
#produto3 <-  "#9e2f24 CAFÉ DELTA Q QHARACTER 10CAP"
#produto4 <-  "#c42d1d CAFÉ GINGA 10CAP"
#produto5 <-  "#b24159 CAFÉ MOÍDO MÁQUINA CHÁVENA DELTA 250G"
...
#produto11 <- "#ad4a23 BEBIDA CEREAIS DELTA C/20%CAFE FR 200G"
...
perc1 <- aggregate(`CAFÉ DELTA Q QALIDUS 10CAP` ~ DATA,DATAFUSION, mean)
G1<-ggplot(perc1, aes(x = DATA, y = `CAFÉ DELTA Q QALIDUS 10CAP`)) + 
...
DATAFUSION$fimQalidus<- ifelse(DATAFUSION$`CAFÉ DELTA Q QALIDUS 10CAP`==0, 0.1, 1)
...
perc3 <- aggregate(`CAFÉ DELTA Q QHARACTER 10CAP` ~ DATA,DATAFUSION, mean)
G3<-ggplot(perc3, aes(x = DATA, y = `CAFÉ DELTA Q QHARACTER 10CAP`)) + 
  geom_col(fill="#9e2f24") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),limits = c(0, 1)) +
  labs(x = "Data - fim de semana", y = "Proporção de presenças em lojas")+theme(panel.background = element_blank())

DATAFUSION$fimQharacter<- ifelse(DATAFUSION$`CAFÉ DELTA Q QHARACTER 10CAP` ==0, 0.1, 1)
...
perc11 <- aggregate(`BEBIDA CEREAIS DELTA C/20%CAFE FR 200G` ~ DATA,DATAFUSION, mean)
G11<-ggplot(perc11, aes(x = DATA, y = `BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) + 
  geom_col(fill="#ad4a23") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),limits = c(0, 1)) +
  labs(x = "Data - fim de semana", y = "Proporção de presenças em lojas")+theme(panel.background = element_blank())

DATAFUSION$fimBebida<- ifelse(DATAFUSION$`BEBIDA CEREAIS DELTA C/20%CAFE FR 200G` ==0, 0.1, 1)
fim11 <- aggregate(`fimBebida` ~ Loja,DATAFUSION, mean)
F11<-ggplot(data = fim11, aes(x = Loja, y = `fimBebida`)) +
  geom_bar(stat = "identity", fill = "dark orange") +
  labs(x = "Loja", y = "Presença do produto no Linear") + theme(axis.text.x = element_blank(), panel.background = element_blank())
...
#FreqC1 <- table(PresençaC1$`SINAL CAFÉ DELTA Q QALIDUS 10CAP`)  #Também não consigo mudar aqui
...
FreqC1 <- table(PresençaC1$`SINAL CAFÉ DELTA Q QALIDUS 10CAP`)
dfPercC1<-data.frame(prop.table(FreqC1))
...
PresençaC3 <- DATAFUSION %>%
  dplyr::select(`SINAL CAFÉ DELTA Q QHARACTER 10CAP`)
PresençaC3<-na.omit(PresençaC3)

FreqC3 <- table(PresençaC3$`SINAL CAFÉ DELTA Q QHARACTER 10CAP`)
...
PresençaC7 <- DATAFUSION %>%
  dplyr::select(`SINAL CAFÉ TORRADO RITUAL DELTA GRÃO  500G`, `Ritual`)
...
FreqC11 <- table(PresençaC11$`SINAL BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)
...
C1 <- ggplot(ciclo1, aes(x = `CICLO CAFÉ DELTA Q QALIDUS 10CAP`)) +
...
ciclo3 <-DATAFUSION %>%
  dplyr::select(`CICLO CAFÉ DELTA Q QHARACTER 10CAP`)
ciclo3<-na.omit(ciclo3)

C3<- ggplot(ciclo3, aes(x = `CICLO CAFÉ DELTA Q QHARACTER 10CAP`)) +
  geom_bar(fill = "#4b625a", position = "stack") +
  labs(title = "`CICLO ASSEGURADO CAFÉ DELTA Q QHARACTER 10CAP`",
       x = "Classes",
       y = "Frequência") + theme(panel.background = element_blank())
...
C11<- ggplot(ciclo11, aes(x = `CICLO BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) +
...
rot1 <- table(rotura1$`ROTURA CAFÉ DELTA Q QALIDUS 10CAP`)
...
rotura3 <-DATAFUSION %>%
  dplyr::select(`ROTURA CAFÉ DELTA Q QHARACTER 10CAP`)
rotura3<-na.omit(rotura3)

rot3 <- table(rotura3$`ROTURA CAFÉ DELTA Q QHARACTER 10CAP`)
...
rot11 <- table(rotura11$`ROTURA BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)
...
Freq1 <- table(adequa1$`ADEQUAÇÃO CAFÉ DELTA Q QALIDUS 10CAP`)
...
adequa3 <-DATAFUSION %>%
  dplyr::select(`ADEQUAÇÃO CAFÉ DELTA Q QHARACTER 10CAP`)
adequa3<-na.omit(adequa3)

Freq3 <- table(adequa3$`ADEQUAÇÃO CAFÉ DELTA Q QHARACTER 10CAP`)
...
Freq11 <- table(adequa11$`ADEQUAÇÃO BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)
...
Ci1<-ggplot(ciclo1, aes(x="", y=`CICLO CAFÉ DELTA Q QALIDUS 10CAP`)) + 
...
Ci3<-ggplot(ciclo3, aes(x="", y=`CICLO CAFÉ DELTA Q QHARACTER 10CAP`)) + 
  geom_violin(trim=FALSE) +
...
Ci11<-ggplot(ciclo11, aes(x="", y=`CICLO BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) + 
  geom_violin(trim=FALSE) +
...
SO1<-ggplot(DATAFUSION, aes(x="", y=`SELLOUT CAFÉ DELTA Q QALIDUS 10CAP`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#98362b", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
SO11<-ggplot(DATAFUSION2022, aes(x="", y=`SELLOUT CAFÉ DELTA Q QALIDUS 10CAP`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#98362b", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
SO3<-ggplot(DATAFUSION, aes(x="", y=`SELLOUT CAFÉ DELTA Q QHARACTER 10CAP`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#9e2f24", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
SO31<-ggplot(DATAFUSION2022, aes(x="", y=`SELLOUT CAFÉ DELTA Q QHARACTER 10CAP`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#9e2f24", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
SO11<-ggplot(DATAFUSION, aes(x="", y=`SELLOUT BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#ad4a23", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
SO111<-ggplot(DATAFUSION2022, aes(x="", y=`SELLOUT BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) + 
  geom_violin(trim=FALSE) +
  geom_boxplot(width=0.1, fill="#ad4a23", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
DR1<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura CAFÉ DELTA Q QALIDUS 10CAP`)) + 
  geom_boxplot(width=0.1, fill="#98362b", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
...
DR3<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura CAFÉ DELTA Q QHARACTER 10CAP`)) + 
...
DR11<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) + 
  geom_boxplot(width=0.1, fill="#ad4a23", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
  labs(x="", y="Dias para a rotura")
...
DRL1<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura_Linear CAFÉ DELTA Q QALIDUS 10CAP`)) + 
  geom_boxplot(width=0.1, fill="#98362b", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
  labs(x="", y="Dias para a rotura")
...
DRL3<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura_Linear CAFÉ DELTA Q QHARACTER 10CAP`)) + 
  geom_boxplot(width=0.1, fill="#9e2f24", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
  labs(x="", y="Dias para a rotura")
...
DRL11<-ggplot(DATAFUSION, aes(x="", y=`Dias_para_Rotura_Linear BEBIDA CEREAIS DELTA C/20%CAFE FR 200G`)) +
  geom_boxplot(width=0.1, fill="#ad4a23", outlier.size=0.5) +
  coord_flip() +
  theme_classic() +
  labs(x="", y="Dias para a rotura")
