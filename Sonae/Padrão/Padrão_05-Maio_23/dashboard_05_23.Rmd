...
```{r}
valueBox(127, icon = "fa-store")
```

### Resumo

Relatório de auditoria a 11 produtos Delta em lojas SONAE.

Fim-de-semana: 26 (sexta), 27 (sábado) e 28 (domingo) de Maio.

Presença mais baixa nos dois produtos de café RITUAL e no GINGA 10 CAPS nos lineares visitados.

Presença deficitária de produtos (abaixo ou igual a 70%) nos Continente Modelo de Almeirim, Fundão, Palmela e Montijo.

Um total de 72 das 127 lojas (57%) apresentaram em prateleira todos os produtos auditados.

Do ponto de vista Global as lojas com reposição registaram ligeiramente menor proporção de produtos ausentes em linear (verificação ninjas).
Contudo, os dois podutos de café Ritual o café CHÁVENA 250g estavam menos presentes em lojas com reposição contratada.
...
# QHARACTER 10 {data-orientation="rows"}
...
```{r}
datatable(DATAFUSION[, c(1:5,18,6:16,173,23,28,33,38,43,48,53,58,63,68,73,19,24,29,34,39,44,49,54,59,64,69)], extensions = c('Buttons', 'SearchBuilder'), options = list(
  dom = 'Bfrtip',
  buttons = list(
    'copy', 
    list(
...
rmarkdown::render("D:\\B&N Dados\\Delta\\Padrão_05_2023\\dashboard_05_23.Rmd")
