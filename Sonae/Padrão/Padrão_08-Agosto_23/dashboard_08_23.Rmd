...
Relatório de auditoria a 10 produtos Delta em lojas SONAE.

Fim-de-semana: 18 (sexta), 19 (sábado) e 20 (domingo) de Agosto.

Presença mais baixa no café DELTA Q QALIDUS 80 nos lineares visitados.

Presença deficitária de produtos (abaixo ou igual a 70%) nos Continentes Portimão, Torres Novas e nos Continentes Modelo de Belas, Castelo Branco.

Um total de 71 das 138 lojas (51%) apresentaram em prateleira todos os produtos auditados.

Houve uma ligeira dominância das lojas sem reposição relativamente à proporção de produtos observados pelos Ninjas.
Houve produtos que tiveram muito maior presença em linear nos pontos de venda sem reposição, como foram os casos do café DELTA Q QHARACTER e do café DELTA Q QALIDUS.
...
# MYTHIC 80 {data-orientation="rows"}
...
# AROMA INTENSO 30 {data-orientation="rows"}
...
# QHARACTER 80 {data-orientation="rows"}
...
# CEVADA 200G {data-orientation="rows"}
...
# BELLISSIMO INTENSO 10 {data-orientation="rows"}
...
# BELLISSIMO ORIGINALE 10 {data-orientation="rows"}
...
# LATTE GO CHILL {data-orientation="rows"}
...
# CAPPUCCINO  CHILL{data-orientation="rows"}
...
# CARAMEL MACCHIATO GO CHILL{data-orientation="rows"}
...
# LATTE MACCHIATO GO CHILL{data-orientation="rows"}
...
```{r}
datatable(DATAFUSION[, c(1:19, 23,28,33,38,43,48,53,58,63,68,73,78,83, 87, 93, 27,32,37,42,47,52,57,62,67,72,77,82,87,92,97)], extensions = c('Buttons', 'SearchBuilder'), options = list(
  dom = 'Bfrtip',
  buttons = list(
    'copy', 
    list(
...
```{r}
datatable(DATAFUSION[,c(1:236)], extensions = c('Buttons', 'SearchBuilder'), options = list(
  dom = 'Bfrtip',
...
rmarkdown::render("D:\\B&N Dados\\Delta\\Padrão\\Padrão_08_2023\\dashboard_08_23.Rmd")
